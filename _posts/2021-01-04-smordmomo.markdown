...
I switched broker to Invast which means I'll be using CFDs for the interim. Since I am doing 24 transactions/month this has the potential to save in transaction costs up until a certain portfolio value (I did some napkin math). Financing rates are approximately 3% per annum at the moment which seems rediculously low. 
...
{% assign mydata=site.data.2021-01-04-smordmomo_invast %}

<table>
    <caption>Beginnings with Invast</caption>
    <thead>
    {% for column in mydata[0] %}
        <th>{{ column[0] }}</th>
    {% endfor %}
